...
async def test_post_instagram_photo(monkeypatch):
    """post_instagram should call Graph API to create a container then publish it."""
    from app.integrations.instagram import tools as igtools

    mock_redis = MagicMock()
    mock_redis.get = AsyncMock(return_value=b'{"access_token": "ig-tok", "ig_user_id": "123456"}')
    monkeypatch.setattr(igtools, "redis_client", mock_redis)
    ...
    with patch("httpx.AsyncClient") as mock_cls:
        ...
        result = await igtools.post_instagram(
            user_id="user-1",
            image_url="https://example.com/photo.jpg",
            caption="My cool post"
        )
    assert "media-xyz" in result
